...
3. Capture the resulting authentication activity from Atlas's local logs.
4. Generate network traffic that can be captured and analyzed later using Wireshark.
5. Generate security events that can subsequently be analyzed using Wazuh.
6. Establish an unmitigated baseline for comparison against the later Fail2Ban mitigation phase.
...
Security monitoring - Wazuh, Centralized detection and visualization of SSH-related events 
Network - Wireshark, Packet-level network activity associated with the experiment


This separation of evidence is important because each tool provides a different view of the same security event. Hydra demonstrates that the attack was actively generated, Atlas records what occurred on the target, Wazuh provides centralized security monitoring, and Wireshark provides network-level evidence.

## Baseline Before Mitigation

No automated blocking mechanism was active during this attack. In particular, Fail2Ban was intentionally disabled so that the behavior of the SSH service under repeated authentication attempts could be observed without mitigation.

This attack therefore establishes the **unmitigated baseline** for the lab.

The resulting evidence will be used in the later mitigation phase to compare the behavior of the system before and after Fail2Ban is enabled.

The comparison will focus on:

- Number of authentication attempts generated
- SSH authentication failures recorded by Atlas
- Events detected and displayed by Wazuh
- Network behavior observed through Wireshark
- Whether the attacker's IP address is subsequently blocked by Fail2Ban
- Changes in attack activity after the defensive control is activated

## Conclusion

The controlled SSH brute-force attack against Atlas was successfully executed from Hades.

The attack generated authentication activity that was recorded locally by Atlas and collected by the Wazuh agent. The same period of activity was also captured at the network level using Wireshark.

This phase establishes the attack scenario and provides the baseline evidence required for the subsequent analysis and mitigation phases.

The next phase will examine the attack from the **SIEM perspective using Wazuh**, including the events generated by the SSH brute-force activity and how those events can be used to investigate the incident.
